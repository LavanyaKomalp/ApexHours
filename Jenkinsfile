...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
				rmsg = sh returnStdout: true, script: "project deploy start --manifest manifest/package.xml -u ${HUB_ORG}"
			}else{
			   rmsg = bat returnStdout: true, script: "project deploy start --manifest manifest/package.xml -u ${HUB_ORG}"
			}
